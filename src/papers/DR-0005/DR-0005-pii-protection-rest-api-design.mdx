...
3. **Backup verification**: If encrypted backups exist, demonstrate that the key version needed to decrypt the erased subject's data has been destroyed. The KMS audit trail (§14.6.3) shows the key destruction event.
...
| Sensitive identifier embedded in path segments | Same exposure model, even if not named verbatim by CWE-598 | Same logging, caching, and propagation risk as query strings | §1, §3, §6, §9 |
| "HTTPS is enough" reasoning | OWASP guidance rejects that assumption | Transport confidentiality does not prevent server-side persistence and re-exposure | §1, §2, §6, §16 |
