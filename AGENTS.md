...
5. **Semantic Color System (Dark-Theme-Compatible)**: All diagram fills **MUST** use alpha-transparent hex colors so they render correctly on both GitHub light and dark themes. Opaque pastel fills (e.g. `#d4edda`) appear as blinding white rectangles on dark backgrounds. Use a strong base color with low alpha (hex suffix `25` ≈ 15% opacity) — the page background bleeds through, producing a soft pastel on light theme and a muted jewel tone on dark theme.
   - *Success/Trusted*: `#2ecc7125` fill, `#2ecc71` stroke (Green)
   - *Active/Informational*: `#3498db25` fill, `#3498db` stroke (Blue)
   - *Human/Warning*: `#f1c40f25` fill, `#f1c40f` stroke (Yellow)
   - *Terminal/Denied*: `#e74c3c25` fill, `#e74c3c` stroke (Red)
   - Do **not** set a `color:` attribute on `classDef` — let Mermaid's theme pick the text color automatically.
   - For ad-hoc fills (non-semantic), use the same pattern: pick the base hue and append `20`–`30` alpha (e.g. `fill:#8e44ad25,stroke:#8e44ad`).
6. **Code Blocks in Nodes**: Avoid using HTML tags (`<pre>`, `<code>`) for code blocks inside Mermaid nodes, as GitHub's native parser may strip them or format them incorrectly. Instead, use non-breaking spaces (`&nbsp;`) for manual indentation within Markdown strings. *Warning*: Do not use HTML entity hyphens (`&#8209;`) to prevent wrapping, as Mermaid has a known parser bug that renders them incorrectly (e.g., `&-`). Use standard hyphens.
...
8. **Right-Side Text Cutoff**: Lengthy sequence diagram text extending over the right edge gets cropped when `messageAlign: left` is active because Mermaid does not account for left-aligned message spillover when calculating canvas width. Prevent this by anchoring a "phantom note" to the rightmost actor using a Braille Blank character (`⠀` / U+2800). Combined with transparent borders from the rule above, this invisibly forces the canvas to expand horizontally. Example: `Note right of <RightmostActor>: ⠀`
9. **Self-Referential Logic**: Avoid placing large logic pseudo-code in standalone or floating `Note` boxes. Instead, represent logic processing as a self-referential arrow (e.g., `Agent->>Agent: Validate Token`) with the pseudocode attached appropriately (e.g., `Note right of Agent`).
10. **Backticks in Sequence Diagrams**: Avoid using Markdown backticks (`` ` ``) for URLs, code blocks, or endpoints inside `sequenceDiagram` elements (messages or notes). The mermaid sequence parser treats them as literal characters; use standard text instead.
