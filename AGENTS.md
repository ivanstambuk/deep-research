...
- **If a defect appears in auto-generated links, anchors, TOCs, reader routes, or other generated rendering, treat it as a pipeline bug first — not a document-text bug.** Before changing article prose, inspect the generation path (`scripts/compile-mdx.js`, `scripts/build-reader-assets.js`, `scripts/cross-reference-links.js`, and related tests) and verify whether the output should have been synthesized automatically from the existing source.
- **Never patch article text just to compensate for a generator defect.** Do not insert manual Markdown links / anchors into `.mdx` solely because the generated `.md`, reader JSON, or published viewer output is wrong. Fix the generator systematically, regenerate the affected outputs, and only change the prose itself if the source text is actually incorrect or the user explicitly asks for a one-off document-level workaround.
...
**.scratch file safety.** `.scratch/` files are gitignored and normally unrecoverable, so deletion is forbidden by default. An agent may delete `.scratch/` content only when the user explicitly authorizes deletion and the exact targets have first been audited as fully integrated, reproducible, obsolete, or intentionally disposable. Before deleting, resolve and report the exact paths, preserve every artifact that still contains unique or unfinished work, and avoid broad targets, unresolved variables, and globs. Never delete the whole `.scratch/` directory merely because some contents are stale. Never use shell commands to rename, truncate, overwrite, or rewrite `.scratch/` files; only targeted editing tools may modify retained artifacts. To modify an existing `.scratch/` file, update it in place with surgical replacements. Do not create `-v2`, `-v3`, `-final`, or similar variants unless the user explicitly asks for parallel alternatives or versioned drafts. Never create a `.scratch/` file with the same filename as an existing one because that destroys the previous version irrecoverably.
