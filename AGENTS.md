...
5. **Semantic Color System**: Standardize diagram states with consistent CSS class definitions:
   - *Success/Trusted*: `#d4edda` fill, `#155724` stroke (Green)
   - *Active/Informational*: `#d6eaf8` fill, `#2980b9` stroke (Blue)
   - *Human/Warning*: `#fff3cd` fill, `#856404` stroke (Yellow)
   - *Terminal/Denied*: `#f8d7da` fill, `#721c24` stroke (Red)
...
7. **Phase Styling in Sequence Diagrams**: `sequenceDiagram` does not natively support background colors for individual `Note` statements. To style distinct background phases, wrap the steps in `rect rgb(...)` blocks and use `themeVariables` (`noteBkgColor: "transparent"`, `noteBorderColor: "transparent"`) in the YAML frontmatter to allow notes to seamlessly inherit the bounding box's background color without rendering ugly borders.
